...
def android():
    ...
    # 1) MainActivity (enprime + telechajman fichye) — pake a swiv appId nan capacitor.config.json
    with open(os.path.join(ROOT, 'capacitor.config.json'), encoding='utf-8') as f:
        app_id = json.load(f)['appId']
    jdir = os.path.join(main, 'java', *app_id.split('.'))
    os.makedirs(jdir, exist_ok=True)
    with open(os.path.join(ROOT, 'MainActivity.java'), encoding='utf-8') as f:
        java = f.read()
    java = re.sub(r'^package\s+[\w.]+;', 'package ' + app_id + ';', java, count=1, flags=re.M)
    with open(os.path.join(jdir, 'MainActivity.java'), 'w', encoding='utf-8') as f:
        f.write(java)
    print('MainActivity.java ranplase (pake ' + app_id + ')')

    # 1b) Vèsyon app la (parèt nan paramèt telefòn nan)
    gradle = os.path.join(ROOT, 'android', 'app', 'build.gradle')
    if os.path.exists(gradle):
        with open(gradle, encoding='utf-8') as f:
            g = f.read()
        g, n1 = re.subn(r'versionCode\s+\d+', 'versionCode ' + str(VERSION_CODE), g, count=1)
        g, n2 = re.subn(r'versionName\s+"[^"]*"', 'versionName "' + VERSION_NAME + '"', g, count=1)
        with open(gradle, 'w', encoding='utf-8') as f:
            f.write(g)
        print('vèsyon ' + VERSION_NAME + ' (' + str(VERSION_CODE) + ') mete' + ('' if (n1 and n2) else ' — AVÈTISMAN: pa jwenn versionCode/versionName'))
    else:
        print('AVÈTISMAN: android/app/build.gradle pa jwenn')
